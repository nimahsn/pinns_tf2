...
class L_BFGS_B:
    """
    Warning: Code copied from https://github.com/okada39/pinn_wave/blob/master/lib/optimizer.py.
    ToDo: Implement LBFGS optimizer using tensorflow_probability.
    ...
    def tf_evaluate(self, x, y):
        ...
        with tf.GradientTape() as tape:
            residual, u_phi, du_dt_psi, u_bndry = self.model.call(x)
            loss = tf.reduce_mean(tf.square(residual)) + tf.reduce_mean(tf.square(u_phi - y[0])) + tf.reduce_mean(tf.square(du_dt_psi - y[1])) + tf.reduce_mean(tf.square(u_bndry - y[2]))
